...
const getData = () => {
    ...
    getEleData(paramscc).then((res) => {
        let resArr = res.data.data.data
        for (let i = resArr.length - 1; i >= 0; i--) {
            chuChenData.xData.push(resArr[i].createTime.slice(5, 16))
            chuChenData.y1Data.push(resArr[i].DTUDataDetail[0].value)
        }
        initEcharts()
    })
}
let chuChenData: any = reactive({
    xData: [],
    y1Data: [],
})
onMounted(() => {
